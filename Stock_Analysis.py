...
temp["買超"] = temp["買進"]-temp["賣出"]
# 計算均買價，均買價為當日買進總價除以總股數
temp["均買價"] = combine_df.groupby(by='券商').apply(lambda x: ((x['買進股數']*x['價格'])/(x['買進股數'].sum())).sum())
# 計算均賣價，均賣價為當日賣出總價除以總股數
temp["均賣價"] = combine_df.groupby(by='券商').apply(lambda x: ((x['賣出股數']*x['價格'])/(x['賣出股數'].sum())).sum())
